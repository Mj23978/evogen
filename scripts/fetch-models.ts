...
type CloudConfig = { url: string; cacheFile: string; type: 'litellm' | 'together' }
...
function mapLiteLLMConfig(modelName: string, modelConfig: any): ModelInfo | undefined {
  // Extract provider
  const providerName: string = modelConfig.litellm_provider || 'unknown';
  const provider: string | undefined = providerMapper[providerName] || undefined;
  if (!providerName || provider === undefined) {
    return
  }
  if (provider === ProviderType.AmazonBedrock) {
    if (modelName.includes("us-west") || modelName.includes("sa-") || modelName.includes("month") || modelName.includes("ca-") || modelName.includes("us-east") || modelName.includes("eu-west") || modelName.includes("eu-east") || modelName.includes("us.") || modelName.includes("eu.") || modelName.includes("ap-n") || modelName.includes("eu-cen")) {
      return
    }
    modelName = modelName.replace("bedrock/", "")
  }
  if (provider === ProviderType.Azure) {
    if (modelName.includes("us/") || modelName.includes("eu/") || modelName.includes("month") || modelName.includes("ca-") || modelName.includes("us-east") || modelName.includes("eu-west") || modelName.includes("eu-east") || modelName.includes("us.") || modelName.includes("eu.") || modelName.includes("ap-n") || modelName.includes("eu-cen")) {
      return
    }
  ...
  if (modelConfig.input_cost_per_token) {
    cost.inputCost = parseFloat((modelConfig.input_cost_per_token * 1000000).toFixed(3));
  }

  if (modelConfig.output_cost_per_token) {
    cost.outputCost = parseFloat((modelConfig.output_cost_per_token * 1000000).toFixed(3));
  }
  ...
  if (modelConfig.search_context_cost_per_query) {
    metadata.searchContextCostPerQuery = parseFloat((modelConfig.search_context_cost_per_query * 1000000).toFixed(3));
  }

  // Add any batch pricing
  if (modelConfig.input_cost_per_token_batches) {
    metadata.inputCostBatches = parseFloat((modelConfig.input_cost_per_token_batches * 1000000).toFixed(3));
  }

  if (modelConfig.output_cost_per_token_batches) {
    metadata.outputCostBatches = parseFloat((modelConfig.output_cost_per_token_batches * 1000000).toFixed(3));
  }
...
function mapTogetherLLMConfig(modelConfig: any): ModelInfo | undefined {
  ...
  const structuredOutputsModels = ["meta-llama/Meta-Llama-3.1-8B-Instruct-Turbo", "meta-llama/Meta-Llama-3.1-70B-Instruct-Turbo", "meta-llama/Llama-3.2-3B-Instruct-Turbo", "meta-llama/Llama-3.3-70B-Instruct-Turbo", "deepseek-ai/DeepSeek-V3"]
  const functionCallModels = ["Qwen/Qwen2.5-7B-Instruct-Turbo", "Qwen/Qwen2.5-72B-Instruct-Turbo", "meta-llama/Meta-Llama-3.1-8B-Instruct-Turbo", "meta-llama/Meta-Llama-3.1-70B-Instruct-Turbo", "meta-llama/Meta-Llama-3.1-405B-Instruct-Turbo", "meta-llama/Llama-3.3-70B-Instruct-Turbo"]
  if (modelName.toLowerCase().includes('vision') || modelLabel.toLowerCase().includes('vision')) {
  ...
  if (modelConfig.pricing.output) {
    cost.outputCost = parseFloat((modelConfig.pricing.output * 0.004).toFixed(3));
  }

  if (modelConfig.pricing.pixelPricing) {
    cost.inputCostPixel = parseFloat((modelConfig.pricing.pixelPricing.pricePerPixelDollar * 1000000).toFixed(3));
  }
...
function readFromCache(cacheFile: string, maxAgeMs: number = 24 * 60 * 60 * 1000): any | null {
  try {
    if (fs.existsSync(cacheFile)) {
      ...
      if (fileAge < maxAgeMs) {
        console.log(`Using cached data (${Math.round(fileAge / 60000)} minutes old)`);
        const cacheData = fs.readFileSync(cacheFile, 'utf8');
        return JSON.parse(cacheData);
      } else {
        console.log(`Cache expired (${Math.round(fileAge / 60000)} minutes old)`);
      }
    }
  } catch (error) {
...
async function downloadAndProcessModels(json: CloudConfig, cacheMaxAgeMs: number = 720 * 60 * 60 * 1000): Promise<void> {
  try {
    ...
    for (const [_, modelConfig] of Object.entries(allModels)) {
      console.log(`Processed ${modelConfig.models.length} ${modelConfig.fileName} models`);
      writeToTsFile(modelConfig.models, `./static/models/${modelConfig.fileName}.ts`);
    }

  } catch (error) {
    if (error instanceof AxiosError && error.response?.status === 401) {
      console.error(`API Key : ${process.env.TOGETHER_API_KEY}`)
      console.error("Unauthorized: Please check your TOGETHER_API_KEY in the environment variables.");
      process.exit(1);
    }
