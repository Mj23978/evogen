import { togetherai, TogetherAIProviderSettings } from "@ai-sdk/togetherai";
import type {
  EmbeddingModelV2,
  ImageModelV2,
  LanguageModelV2,
...
type TogetherAIApiResponse = {
  models: TogetherAIModel[];
};

export class TogetherAIProvider extends BaseEvogenProvider<TogetherAIProviderSettings> {
  ...
  async syncModelsFromServer(
    ...
    const data = (await response.json()) as TogetherAIApiResponse;

    const models = data.models.map<ModelInfo>((model: TogetherAIModel) => {
      const {
        name,
        endpoints,
        context_length,
...
export function parseTogetherAIConfig(
  config: Record<string, any>
): TogetherAIProviderSettings {
  if (!config || !config.apiKey) {
    throw new Error("Missing API key in TogetherAI configuration.");
  }
